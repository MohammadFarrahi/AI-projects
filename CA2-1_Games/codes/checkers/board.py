...
class Board:
    ...
    def getValidMoves(self, piece):
        right, left = piece.col + 1, piece.col - 1
        moves = {}
        if piece.color == WHITE or piece.king:
            moves.update(self._traverseLeft(piece.row+1, min(piece.row+3, ROWS), 1, piece.color, left))
            moves.update(self._traverseRight(piece.row+1, min(piece.row+3, ROWS), 1, piece.color, right))
        if piece.color == RED or piece.king:
            moves.update(self._traverseLeft(piece.row-1, max(piece.row-3, -1), -1, piece.color, left))
            moves.update(self._traverseRight(piece.row-1, max(piece.row-3, -1), -1, piece.color, right))
        return moves
